...
serve(async (req) => {
  ...
  try {
    // Supabase automatically verifies the Authorization header (JWT).
    // We can extract the user info if needed, but for now we just verify it exists.
    ...
    if (!text || !persona) {
      throw new Error('Missing text or persona')
    }
    ...
    // Get the GROQ API KEY securely from Supabase Secrets
    const groqApiKey = Deno.env.get('GROQ_API_KEY')
    if (!groqApiKey) {
      throw new Error('Server configuration error: missing GROQ_API_KEY secret')
    }
    ...
    // Clean up response markdown if it exists
    rewrittenText = rewrittenText.replace(/^\`\`\`html\n|^\`\`\`\n|\`\`\`$/g, '');

    return new Response(
      JSON.stringify({ text: rewrittenText.trim(), isTruncated }),
